...
- (void)updateMetadataWithTitle:(NSString *)title
    ...
    // 测试封面：红蓝渐变（验证灵动岛封面流动背景用）
    CGSize size = CGSizeMake(200, 200);
    NSImage *cover = [[NSImage alloc] initWithSize:size];
    [cover lockFocus];
    NSGradient *grad = [[NSGradient alloc] initWithColors:@[
        [NSColor colorWithRed:0.9 green:0.2 blue:0.3 alpha:1.0],
        [NSColor colorWithRed:0.2 green:0.3 blue:0.9 alpha:1.0]
    ]];
    [grad drawInRect:NSMakeRect(0, 0, 200, 200) angle:45];
    [cover unlockFocus];
    nowPlayingInfo[MPMediaItemPropertyArtwork] = [[MPMediaItemArtwork alloc]
        initWithBoundsSize:size
        requestHandler:^NSImage *_Nonnull(CGSize s) { return cover; }];
